...
app.use(express.static(__dirname)); // Ensures Vercel knows exactly where your static files live

// SQLite DB Configuration (Uses /tmp on Vercel to avoid read-only system errors)
const dbPath = process.env.NODE_ENV === 'production' ? '/tmp/portfolio.db' : path.join(__dirname, 'portfolio.db');
...
const db = new sqlite3.Database(dbPath, err => {
  if (err) {
    ...
    // Initialize database tables once the connection is established
    initializeDatabase();
  }
});
...
function initializeDatabase() {
  db.serialize(() => {
    ...
    // Default admin initialization safely handled
...
// Add admin auth middleware (STRAY WORD REMOVED - FIXED!)
...
// Only run app.listen locally (not on Vercel's Serverless handling)
if (process.env.NODE_ENV !== 'production') {
  app.listen(PORT, () => console.log(`Server running locally on http://localhost:${PORT}`));
}

// Export the app instance for Vercel Serverless Function targeting
module.exports = app;